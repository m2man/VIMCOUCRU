# --- NOTE --- 
# (MEAN BURDEN) 
# This file is used to generate cases and deaths from SERIES POSTERIOR_FOI$LAMBDA, THEN TAKE MEAN OF CASES AND DEATHS
...
cat('===== START [Create_Burden_MeanBurden.R] =====\n')
...
for (idx_region in 1 : length(regions_vector)){
    ...
    for (idx_year in start_year_column : ncol(Naive.Cases)){
        ...
        for (idx.age in 1 : nrow(NaivePop.Region)){
            ...
            # Naive
            Naive.Cases[[idx_year]][idx.age] <- mean(naive.cases.temp)
            Naive.Immuned[[idx_year]][idx.age] <- mean(naive.immuned.temp)
            Naive.Deaths[[idx_year]][idx.age] <- mean(naive.deaths.temp)
            Naive.DALYs[[idx_year]][idx.age] <- mean(naive.DALYs.temp)
            
            # Routine
            Routine.Cases[[idx_year]][idx.age] <- mean(routine.cases.temp)
            Routine.Immuned[[idx_year]][idx.age] <- mean(routine.immuned.temp)
            Routine.Deaths[[idx_year]][idx.age] <- mean(routine.deaths.temp)
            Routine.DALYs[[idx_year]][idx.age] <- mean(routine.DALYs.temp)
            
            # Campaign
            Campaign.Cases[[idx_year]][idx.age] <- mean(campaign.cases.temp)
            Campaign.Immuned[[idx_year]][idx.age] <- mean(campaign.immuned.temp)
            Campaign.Deaths[[idx_year]][idx.age] <- mean(campaign.deaths.temp)
            Campaign.DALYs[[idx_year]][idx.age] <- mean(campaign.DALYs.temp)
        }
    }
...
if (save_file){
    saveRDS(naive.list, paste0(Savepath, 'Naive_Burden_MeanBurden.Rds'))
    saveRDS(routine.list, paste0(Savepath, 'Routine_Burden_MeanBurden.Rds'))
    saveRDS(campaign.list, paste0(Savepath, 'Campaign_Burden_MeanBurden.Rds'))    
}

cat('===== FINISH [Create_Burden_MeanBurden.R] =====\n')